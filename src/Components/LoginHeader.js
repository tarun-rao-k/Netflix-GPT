...
import { removeTmdbResult, setShowGptSearchPage } from '../Utilities/gptSlice.js';
...
const LoginHeader = () => {
  ...
  return (
    <div className='bg-gradient-to-b from-black to-transparent absolute w-full flex items-center justify-between z-10'>
      <div>
      ...
      {user&&<div className='flex items-center'>
       {selector&&<select onChange={(e)=>languageChanged(e.target.value)}>
          {languageOptions.map((languageOption)=>{
              return <option key={languageOption.value} value={languageOption.value} >{languageOption.language}</option>
          })}
          
        </select>}
        <button className='bg-orange-500 text-white px-2 py-1 mr-4 rounded-lg ml-2' onClick={handleGptPage}>{selector?"Home page":"GPT Search"}</button>
